...
part1_q1 = r"""
First of all, RNNs process sequences step-by-step, thus the entire sequence needs to be loaded into memory. 
Since the entire corpus might be very large, processing it as a single sequence may require large storage resources, 
and might even be impossible. 
 Splitting the corpus and loading only a part of the corpus every time is much more memory 
efficient and feasible.


Secondly, splitting the corpus into sequences enables processing of multiple sequences in parallel, improving efficiency 
...
part1_q2 = r"""
When generating text, the model takes into consideration the previous hidden state generated by the model,
...
part3_q1 = r"""
When stacking encoder layers that use sliding-window attention, the resulting broader context in the final layer can be understood by drawing an analogy to the stacking of CNN (Convolutional Neural Network) layers.
First, we recall that the receptive field of some neuron in a CNN refers to the region in the input space that that particular feature is affected by.
Therefore, the different features that are learned in different layers of a CNN are bounded to their receptive field, 
meaning every feature of the network can learn patterns/structures/relationships only from pixels within it's receptive field.
As we stack more layers, the receptive field of each neuron should increases, enabling it to capture larger patterns and learn more global representations of the input data. 

Similarly, when stacking encoder layers that use the sliding-window attention, 
the attention mechanism in each layer focuses on a local context window around each token and aggregates information only from tokens that are within that window. 
As we stack more encoder layers, the attention mechanism has the opportunity to learn from a broader context by iteratively capturing dependencies over larger 
regions of the input sequence, and gradually integrate information from a wider range of tokens, similar to how the receptive field of filters in a CNN expands with each layer.

Therefore, stacking encoder layers with sliding-window attention results in a broader context in the final layer, that can capture long-range dependencies in the input sequence.
"""

part3_q2 = r"""
One variation that can provide a more global context while maintaining similar computational complexity to 
the sliding-window attention is the "Sparse Attention" mechanism.
This mechanism includes sampling a subset of tokens for attention computation, reducing the number of pairwise dot products 
while capturing more global dependencies within the input sequence.
This idea shares similarities with convolutional layers with dilation: 
dilation introduces gaps between the values in a convolutional filters or kernels. 
It allows for an increased receptive field without increasing the number of parameters or the computational cost significantly.
The key benefit of these techniques is the ability to capture larger context while maintaining a relatively low computational cost,
that by appropriate tunning can be similar to that of the sliding-window attention mechanism. 
"""
...
part4_q2 = r"""
first we note that the correct wording of the question (as published on Piazza) is:
"Assume that when fine-tuning, instead of freezing all the layers besides the last two linear layers, you instead left some other internal model layers unfrozen,
 such as the multi-headed attention blocks.
Would the model still be able to successfully fine-tune to this task?   
Or would the results be worse?  
Explain"


Theoretically, the model can still be successfully fine-tuned after this change, 
but we claim that freezing early inner layers of a pretrained model during fine-tuning can have a potential benefits 
compared to unfreezing them.
Generally, the lower layers of a language model capture more general and universal patterns/structures (features) 
and therefore can be highly relevant for different downstream tasks and datasets.
By freezing layers which belong to the feature extractor (encoder layers), we transfer a wide learned linguistic knowledge, even for tasks with limited amounts of data.
On the other hand, the deeper layers in the model usually capture more complex features- task-specific patterns, which 
can be more sensitive to the characteristics of dataset.
This behavior (of deep neural networks in general) encourages us to assume that unfreezing early layers of the model may 
cause a phenomena known as "catastrophic forgetting", meaning the model drastically forgets previously learned information 
during the fine-tune phase on the target task. This can explain why we can get worst results by additionally 
unfreezing early inner layers. 

Meanwhile, when the pre-task and the target task are relatively different, it may be the case that the more
 complex representation transferred by the language model does not capture relevant nuances to the downstream task.
 In these cases, unfreezing additional deep inner layers such as multi-headed attention blocks, may improve the model 
 performance by allowing it to adapt and learn task-specific representations that potentially better align with the target task's data distribution.
